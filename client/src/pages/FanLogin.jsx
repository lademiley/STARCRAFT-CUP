...
export default function FanLogin() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      navigate(data.fan.status === 'pending_payment' ? '/fan/checkout' : '/fan/dashboard')
    } catch {
      ...
      <div className="auth-container" style={{ maxWidth: 420 }}>
        <div className="auth-card card">
          <div className="auth-logo">
            ...
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: '0.9rem', fontWeight: 900, letterSpacing: '3px', color: 'var(--gold)' }}>FAN LOGIN</div>
          </div>
          <h2 style={{ textAlign: 'center', marginBottom: 6 }}>Welcome Back</h2>
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)', marginBottom: 24 }}>Sign in to your fan account</p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="form-group">
              ...
              <input type="email" className="form-control" placeholder="you@email.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} autoComplete="email" required />
            </div>
            <div className="form-group">
          ...
          <p style={{ textAlign: 'center', fontSize: '0.9rem', color: 'rgba(255,255,255,0.5)' }}>
            Don't have an account? <Link to="/register/fan" style={{ color: 'var(--gold)', fontWeight: 700 }}>Register →</Link>
          </p>
        </div>
      </div>
